<template>
  <nav class="w-20 bg-zinc-100 dark:bg-zinc-900 flex-shrink-0 flex flex-col items-center pt-3 pb-3 border-r border-zinc-200 dark:border-zinc-800"
       aria-label="Primary">
    <RouterLink to="/mailbox" class="w-full flex flex-col items-center mb-1" :aria-label="t('nav.mailbox')">
      <div :class="navClass('/mailbox')">
        <div class="w-12 h-10 flex justify-center items-center">
          <PhMailbox :size="24" :weight="route.path === '/mailbox' ? 'fill' : 'duotone'"/>
        </div>
        <div class="text-[11px] font-medium uppercase tracking-wide text-center select-none">{{ t('nav.mailbox') }}</div>
      </div>
    </RouterLink>
    ...
    <RouterLink to="/settings" class="w-full flex flex-col items-center mb-1" :aria-label="t('nav.settings')">
      <div :class="navClass('/settings')">
        <div class="w-12 h-10 flex justify-center items-center">
          <PhGearSix :size="24" :weight="route.path === '/settings' ? 'fill' : 'duotone'"/>
        </div>
        <div class="text-[11px] font-medium uppercase tracking-wide text-center select-none">{{ t('nav.settings') }}</div>
      </div>
    </RouterLink>
    ...
    <div class="mt-auto flex flex-col items-center gap-1">
      ...
      <div class="relative">
        ...
        <div v-if="menu === 'lang'"
             class="absolute bottom-0 left-14 z-50 w-36 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-lg py-1">
...
function navClass(path) {
  ...
  return `relative mb-3 py-1.5 w-16 rounded-lg transition-all duration-150 cursor-pointer ${
    active
